...
void * handle_client(void * arg){
	...
	struct dns_hdr * header = (struct dns_hdr *)client->dns_packet;
	
	//Get Domain from query
	char * question_domain=get_hostname_from_query(client->dns_packet, client->packet_size);
	...
	int send_count = sendto(client->sock, client->dns_packet, client->packet_size, 0, (struct sockaddr*)&name_server, sizeof(struct sockaddr_in6));
	...
	int response_size;
	if((response_packet=receive(client->sock, SOCK_DGRAM, &response_size, &server_address))==NULL){
		perror("Receive Failed");
		return EXIT_FAILURE;
	}
	
	//send the response to client
	send_count = sendto(client->sock, response_packet, response_size, 0, (struct sockaddr*)client->addr, sizeof(struct sockaddr_in6));

	if(question_domain)
		free(question_domain);
	if(response_packet)
	...
	if(client->addr)
		free(client->addr);
	if(client->dns_packet)
		free(client->dns_packet);
	if(client)
		free(client);
...
int main(int argc, char ** argv){
	...
	while(1){
		...
		int maxfdp=sockfd+1;
		switch(select(maxfdp,&fds,&fdsw,NULL,&timeout)){
			case -1: 
				exit(-1);
				break;
			case 0: 
			...
			default:
				if(FD_ISSET(sockfd, &fds)){
					...
					struct client_state * client = (struct client_state*)malloc(sizeof(struct client_state));
					if(client==NULL){
						perror("Malloc Failed");
						return EXIT_FAILURE;
					}
					...
					client->sock = sockfd;
					client->addr = (struct sockaddr_in6 *)malloc(sizeof(struct sockaddr_in6)); 
					if(client->addr==NULL){
						perror("Malloc Failed");
						return EXIT_FAILURE;
					}
					memcpy(client->addr,&client_address, sizeof(struct sockaddr_in6));

					client->packet_size=packet_size;
					client->dns_packet = malloc(sizeof(char)*packet_size);
					if(client->dns_packet==NULL){
						perror("Malloc Failed");
						return EXIT_FAILURE;
					}
					memcpy(client->dns_packet, dns_packet, packet_size);
